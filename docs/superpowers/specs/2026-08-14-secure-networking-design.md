...
Use OpenMLS `0.9.0-rc.2` behind a project-owned Rust `staticlib` and a minimal C ABI. OpenMLS `0.8.1` is rejected because its constrained dependency closure retains `libcrux-sha3 0.0.8` and `libcrux-secrets 0.0.5`, affected by RUSTSEC-2026-0207/0208 and RUSTSEC-2026-0212. The release candidate resolves to the patched generations and is pre-release-gated until a clean stable release or independent audit. The bridge exposes domain operations rather than OpenMLS internals. OpenMLS debug features that can print content or key material are forbidden. The initial mandatory ciphersuite is `MLS_128_DHKEMX25519_CHACHA20POLY1305_SHA256_Ed25519`. Dependency updates require changelog and advisory review, storage-format migration testing, and protocol interoperability fixtures.
