...
void QtScreenCapture::start(const ScreenShareSource &source)
{
    ...
    // Made with the first share through Qt Multimedia rather than with the
    // object: a capture session is what loads Qt Multimedia's backend, and a
    // call controller owns one of these from startup whether or not anything
    // is ever shared. Native capture never needs it.
    if (!m_session) {
