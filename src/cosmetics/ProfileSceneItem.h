...
// `readableRect` region so the name and status line stay legible over it, and
// the `controlRect` region the same way for a control drawn over the scene.
class ProfileScene : public QQuickPaintedItem
{
    ...
    Q_PROPERTY(QRectF controlRect READ controlRect WRITE setControlRect NOTIFY controlRectChanged)
  ...
  public:
    ...
    [[nodiscard]] QRectF controlRect() const { return m_controlRect; }
    void setControlRect(const QRectF &rect);
