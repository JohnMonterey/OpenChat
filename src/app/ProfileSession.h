...
public:
  ...
  // Replaces the provisional account id generated by create() with the account
  // a password login resolved the username to, durably. Only valid on an
  // unlocked session BEFORE startNetworking(): the SyncEngine addresses the
  // relay by the account id it was built with, so a later change is refused.
  [[nodiscard]] Result<void, ProfileSessionError>
  adoptAccountId(const AccountId &accountId);
