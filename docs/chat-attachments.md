...
- **The attach menu** rises from the button: *Photo* (JPG, PNG, WebP),
...
- **Dropping and pasting.** Local files dropped anywhere on the open chat are
  ...
  Shift+Insert with a copied picture, or with files copied in the file
  manager, stages them; any other paste is ordinary text.
- **In the chat**:
...
The nonce is made of the type and index, and the header is the associated
data, so a body cannot be moved to another attachment, index or type. Types
are *part* (224 KiB of the blob), *preview* (a small JPEG), *request* (a
bitmap of the parts a receiver still lacks) and *cancel*.
...
frame size matches the descriptor, a video sequence whose every segment
decodes, or a song container within the chat limits. Previews must be JPEGs of
at most 16 KiB and 320 px. File names are cleaned again on arrival
...
- **Receiving.** Each part is opened with the key before it is kept. Frames
  ...
  within 32 MiB per sender and a day, and are checked when the message
  arrives. When every part is in, a worker thread assembles and checks the
...
video or audio file and two others at a time. A photo, video or audio file
that cannot be converted but fits 16 MiB is sent as a file instead.
...
- **The relay needs no change.** Both parts travel as envelope kinds it
  already carries. The relay and the client now also treat any frame
  received as a sign of life, so a large catch-up on a slow link no longer
  trips the 10-second heartbeat; redeploying the relay is recommended but not
  required.
- **0.2.8 and 0.2.9** consume both parts without showing anything: the
  message is a tagged type they do not know, and a frame is not an MLS
  message they can open.
- **0.1.x** shows the message as one short line of unreadable text, as it
  already does for replies.
...
- `tst_attachment`: the frame codec and sealing (every tamper case), the
  descriptor rules, file-name cleaning, the video sequence codec, blob and
  preview checks, and the chat song limits.
...
- `tst_attachmenttransfer`: two real peers — photos and a 16 MB file end to
  end, groups with a busy link, reordering, loss and recovery, restarts,
  calls, cancelling, budgets and cleanup.
- `tst_chatattachmentimport`: preparing real photos, videos (needs ffmpeg and
  libvpx), audio and files, and the fallbacks.
- `tst_chatcontroller`, `tst_models`: the tray, sending, retry, summaries.
- `tst_qmlload`: the composer layout, the menu, the tray, dropping and
  pasting, every kind of bubble in light and dark and under skins, and the
  viewer. `OPENCHAT_ATTACHMENT_CAPTURES=<dir>` saves its screenshots.
