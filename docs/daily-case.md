...
one of them with the published odds. A case can be opened once an hour: the next
one is available `caseCooldownSeconds` (one hour) after the last was opened, and
the popup and the icon's tooltip say at what time. The drawn item joins the
...
destroyed when closed. The controller has Available, Opening, and Opened
...
claim ID, presentation seed, the time the next case is available and the case's
key in an atomic JSON file under the application's local data directory, keyed
by a hash of the account ID. A nonblocking per-account process lock serializes
status/claim transactions. Repeated claims within the hour return the existing
result. Different local accounts are independent. Preview windows use a
separate `preview` key. Corrupt/unwritable files fail closed with a retryable UI
error. The refresh when the hour is up is a single shot timer; there is no
polling while closed. A claim saved by the once-a-day build waits for midnight,
further off than any hourly claim can; it is released rather than honoured.
...
conventions. The relay currently has no case endpoint. Add a server
transaction, serialized per account, that allows a claim only once
`nextAvailableAt` of the last one has passed on server time. Verify
...
`rewardId`, `seed`, `nextAvailableAt`, the case key and the account's owned
ids. The seed controls only the
...
and stores that key with the claim, so the key, and the belt, do not change
when the claim lands, stay the same in every window and across restarts, and
change only when the next case is available. The seed still only chooses which
of five slots the winner occupies.
...
padlock, with a tooltip pointing at the hourly case; each section counts
